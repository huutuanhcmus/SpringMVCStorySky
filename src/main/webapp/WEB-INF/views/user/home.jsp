...
<body>
	<form action="<c:url value='/truyen'/>" id="formSubmit" method="get">
		...
		<c:forEach var="item" items="${categoryList.listResult}">
			<h1>${item.name}</h1>
			<div id="${item.code}" class="carousel slide"
				data-ride="carousel">
					...
					<c:forEach var="item1" items="${item.listStory}" varStatus="count">
						<c:if test="${count.count <= 1}">
							<div class="carousel-item active" align="middle">
								<img class = "img" onclick="image(${item1.id})" src="${item1.image}"
						...
						<c:if test="${count.count > 1}">
							<div class="carousel-item" align="middle">
								<img  class = "img" onclick="image(${item1.id})"
									src="${item1.image}" alt="Los Angeles" width="auto"
				...
				<a class="carousel-control-prev" href="#${item.code}"
					data-slide="prev"> <span class="carousel-control-prev-icon"></span>
				</a> <a class="carousel-control-next" href="#${item.code}"
